; (function ($) {
    $.extend({
        "login": function () {
            ...
            }, $userTip = $('#uin_tips'), $passTip = $('#pwd_tips'), $operateTip = $('#operate_tips'), $delUserInput = $('#uin_del'), $u = $('#u'), $p = $('#p'),$k = $('#k'),$v = $('#validateCode'), $switch = $('#qrswitch_logo'), $qr = $('#web_qr_login_show');
            $(function () { showLogin(); });
            ...
            $login.click(function () {
                if (!$u.val()) {
                    ...
                    $errTip.find('#err_m').text('请输入验证码！').end().show();
                    setTimeout(function () { $errTip.hide(); }, 5000);
                    return false;
                }else {
                    $('#loading_tips').show();
                    var encrypt = new JSEncrypt();
                    encrypt.setPublicKey('MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCLEge0QzDc4P/QkZh2qzvWfX74wMdMwRro8c15R+yW297bnEB8tzqHjl0RxndmzXGJVZG7iJalL2VuT/66+MfJmSrg6lLbGyI9e726FqSlL4OURi4JaXvT2joSMucCjXK31HwZuDsyxRfs+V4OZc/DA5D8MxI6acfoPuYNzYp/1wIDAQAB');
                    var passwd = $p.val();
                    var validateCode = $k.val();
                    passwd = encrypt.encrypt(validateCode+passwd);
                    $p.val(passwd);
